...
function AvailabilityBadge() {
  ...
  return (
    <div className={cn(
      ...
      <span className={cn('w-2 h-2 rounded-full', isAvailable ? 'bg-success animate-pulse' : 'bg-warning')} />
...
const INFO = [
  ...
  { icon: Phone, label: 'WhatsApp', value: COMPANY.phone, href: `https://wa.me/${COMPANY.phone.replace(/\s+/g, '')}` },
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-slate-600 dark:text-slate-400 leading-relaxed">
          Que vous ayez besoin de visuels produits ou d'une solution digitale complète, notre équipe est là pour transformer votre vision.
        </p>
      </div>
          ...
          >
            ...
            <div className="min-w-0">
              ...
              {item.href ? (
                ...
                >
                  {item.value} <ArrowUpRight className="w-3 h-3 opacity-0 group-hover:opacity-100 transition-all" />
                </a>
              ) : (
      ...
      <a
        href={`https://wa.me/${COMPANY.phone.replace(/\s+/g, '')}?text=Bonjour%20Solinexia%2C%20je%20souhaite%20en%20savoir%20plus%20sur%20vos%20services.`}
        target="_blank"
        rel="noopener noreferrer"
        className="flex items-center justify-center gap-3 w-full px-6 py-4 bg-[#25D366] hover:bg-[#20bd5a] text-white font-semibold rounded-2xl transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-[#25D366]/20"
      >
      ...
      <div className="flex flex-wrap gap-2 pt-4 border-t border-slate-200 dark:border-slate-800">
        <span className="text-xs text-slate-500 w-full mb-1 font-mono">SUIVEZ-NOUS</span>
        {SOCIAL_LINKS.map((s) => (
          <a
            key={s.label}
            href={s.href}
            target="_blank"
            rel="noopener noreferrer"
            className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800/50 rounded-lg text-xs text-slate-600 dark:text-slate-400 hover:text-primary dark:hover:text-primary hover:border-primary/30 transition-all border border-slate-200 dark:border-slate-800 flex items-center gap-1"
          >
